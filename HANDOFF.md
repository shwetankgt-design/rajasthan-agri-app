...
### Database — Neon PostgreSQL (migrated from SQLite, Sept 2026)

The app runs on a **Neon Postgres** database provisioned through the linked Vercel project. `prisma/dev.db` is the old SQLite file and is **no longer used** — it is left on disk only as a fallback.

- Connection strings live in `.env` and `.env.local`. Both are gitignored (`.gitignore` covers `.env*`).
- **`.env.local` is regenerated by `vercel env pull .env.local --environment=development`** — that is the source of truth from Vercel/Neon.
- **`.env` must also carry `DATABASE_URL`**, because the Prisma CLI reads `.env` and *not* `.env.local`. If the Neon credentials rotate, re-pull and copy `DATABASE_URL` + `DATABASE_URL_UNPOOLED` across, or `prisma migrate`/`seed`/`studio` will fail while the Next.js app keeps working — a confusing split.
- Schema changes use **migrations**, not `db push`: `npx prisma migrate dev --name <change>`. The Vercel build runs `prisma migrate deploy && next build`.
- **The Neon database is already seeded** with the same demo data as the old SQLite file (200 farmers, 400 plots, 3 FPOs, 12 contracts…). `npm run seed` wipes and regenerates **the Neon database** — it is no longer a local-only action, so treat it as destructive.

**Performance note:** every query now crosses the network to Neon (~240 ms round trip from India). The government dashboard issues several large fetches and takes **~5 s** to load locally, against ~250 ms on the old SQLite file. Running on Vercel removes most of this because the app sits next to the database. If local speed becomes a problem, the fix is to push aggregation into SQL (`groupBy`/`$queryRaw`) in `governmentAnalytics.ts` and `governmentStats.ts` instead of pulling hundreds of rows and reducing in JS, and to drop the duplicate `getMspGap()` call (the page calls it directly *and* `getHeadlineKpis()` calls it internally).
